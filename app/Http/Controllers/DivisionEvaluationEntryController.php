...
use App\Models\DivisionEvaluationEntry;
...
class DivisionEvaluationEntryController extends DefaultController
{
    protected $modelClass = DivisionEvaluationEntry::class;
    protected $title;
    ...
    {
        $this->title = 'Division Evaluation Entry';
        $this->generalUri = 'division-evaluation-entry';
        // $this->arrPermissions = [];
        $this->actionButtons = ['btn_edit', 'btn_show', 'btn_delete'];

        $this->tableHeaders = [
                    ['name' => 'No', 'column' => '#', 'order' => true],
                    ['name' => 'Division evaluation id', 'column' => 'division_evaluation', 'order' => true],
        ...
        $this->importExcelConfig = [ 
            'primaryKeys' => ['division_evaluation_id'],
            'headers' => [
                    ['name' => 'Division evaluation id', 'column' => 'division_evaluation_id'],
    ...
    protected function fields($mode = "create", $id = '-')
    {
        ...
        $evaluationOptions = DivisionEvaluation::join('evaluators', 'evaluators.id', '=', 'division_evaluations.evaluator_id')
            ...
            ->select([
            'division_evaluations.id as value', 
        ...
        $fields = [
                    [
                        'type' => 'select2',
                        'label' => 'Division evaluation',
                        'name' =>  'division_evaluation_id',
                        'class' => 'col-md-12 my-2',
                        'required' => $this->flagRules('division_evaluation_id', $id),
                        'value' => (isset($edit)) ? $edit->division_evaluation_id : '',
    ...
    {
        $rules = [
                    'division_evaluation_id' => 'required|string',
    ...
    protected function defaultDataQuery()
    {
        ...
        $dataQueries = $this->modelClass::where($filters)
            ->join('kpi_templates', 'kpi_templates.id', '=', 'division_evaluation_entries.kpi_template_id')
            ->join('division_evaluations', 'division_evaluations.id', '=', 'division_evaluation_entries.division_evaluation_id')
            ->join('evaluators', 'evaluators.id', '=', 'division_evaluations.evaluator_id')
            ->join('users as evaluator_users', 'evaluator_users.id', '=', 'evaluators.user_id')
            ->join('users as evaluated_users', 'evaluated_users.id', '=', 'division_evaluations.division_id')
            ->where(function ($query) use ($orThose) {
                $query->where('kpi_templates.title', 'LIKE', '%' . $orThose . '%');
                $query->orWhere('division_evaluation_entries.key_perf_area', 'LIKE', '%' . $orThose . '%');
            ...
            ->select(
                'division_evaluation_entries.*',
                'evaluated_users.name as user',
                'evaluator_users.name as evaluator',
                'kpi_templates.title as kpi_template',
                DB::raw('CONCAT_WS(" / ", evaluation_date,  evaluator_users.name, evaluated_users.name) as division_evaluation')
            );
    ...
    protected function store(Request $request)
    {
        ...
        try {
            ...
            $evaluation = DivisionEvaluation::find($insert->division_evaluation_id);
            if ($evaluation) {
                $totalWeight = DivisionEvaluationEntry::where('division_evaluation_id', $evaluation->id)->sum('weight');
                $finalScore = DivisionEvaluationEntry::where('division_evaluation_id', $evaluation->id)
    ...
    protected function update(Request $request, $id)
    {
        ...
        try {
            ...
            // Update total_weight and final_score in kpi_evaluations table
            $evaluation = DivisionEvaluation::find($change->division_evaluation_id);
            if ($evaluation) {
                $totalWeight = DivisionEvaluationEntry::where('division_evaluation_id', $evaluation->id)->sum('weight');
                $finalScore = DivisionEvaluationEntry::where('division_evaluation_id', $evaluation->id)
                    ->select(DB::raw('SUM((actual / target) * weight) as final_score'))
    ...
    {
        $singleData = $this->defaultDataQuery()->where('division_evaluation_entries.id', $id)->first();
        unset($singleData['id']);

        // hapus yang tidak perlu ditampilkan
        unset($singleData['division_evaluation_id']);
